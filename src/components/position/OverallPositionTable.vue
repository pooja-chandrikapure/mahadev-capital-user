<template>
  <div class="bg-card-background border border-primary-border rounded-xl p-5">
    ...
    <table class="w-full mt-4">
      <thead>
        <tr class="border-b border-primary-border">
          ...
          <th class="text-left text-secondary-text text-xs font-medium py-2 pb-3">BROKER NAME</th>
          <th class="text-left text-secondary-text text-xs font-medium py-2 pb-3">AVG BUY/SELL</th>
          <th class="text-left text-secondary-text text-xs font-medium py-2 pb-3">Qty</th>
          <th class="text-left text-secondary-text text-xs font-medium py-2 pb-3">LTP</th>
      ...
      <tbody>
        ...
        <template v-if="store.isLoading">
          <tr v-for="n in 5" :key="'skel-' + n" class="border-b border-primary-border last:border-0">
            <td class="py-4">
              <div class="h-3.5 w-44 bg-black/10 rounded animate-pulse mb-1"></div>
              <div class="h-2.5 w-16 bg-black/10 rounded animate-pulse"></div>
            </td>
            <td class="py-4"><div class="h-3.5 w-20 bg-black/10 rounded animate-pulse"></div></td>
            <td class="py-4">
              <div class="h-3.5 w-32 bg-black/10 rounded animate-pulse mb-2"></div>
              <div class="h-3.5 w-10 bg-black/10 rounded animate-pulse"></div>
            </td>
            <td class="py-4"><div class="h-3.5 w-10 bg-black/10 rounded animate-pulse"></div></td>
            <td class="py-4"><div class="h-3.5 w-16 bg-black/10 rounded animate-pulse"></div></td>
            <td class="py-4"><div class="h-6 w-16 bg-black/10 rounded animate-pulse"></div></td>
            <td class="py-4"><div class="h-3.5 w-16 bg-black/10 rounded animate-pulse"></div></td>
            <td class="py-4 flex justify-end"><div class="h-7 w-14 bg-black/10 rounded animate-pulse"></div></td>
          </tr>
        </template>
        ...
        <template v-else>
          ...
          >
            ...
            <td class="py-2">
              <div class="flex items-center gap-2">
                <p class="text-secondary-text text-sm font-semibold">{{ position.tradingsymbol }}</p>
                <span class="text-secondary-text bg-table-header-background text-xs border border-primary-border px-1.5 py-0.5 rounded">{{ position.exchange }}</span>
              </div>
              <p class="text-secondary-text text-xs mt-0.5">{{ position.product }}</p>
            </td>

            <!-- Broker Name -->
            <td class="py-2 text-secondary-text text-sm">{{ position.broker_name }}</td>

            <!-- Avg Buy / Sell -->
            <td class="py-2">
              <div class="flex items-center gap-2 mb-1">
                <span class="bg-primary-green text-white text-xs font-bold w-5 h-5 flex items-center justify-center rounded">B</span>
                <span class="text-secondary-text text-sm">{{ position.buy_price > 0 ? position.buy_price : '--' }}</span>
              </div>
              <div class="flex items-center gap-2">
                <span class="bg-primary-red text-white text-xs font-bold w-5 h-5 flex items-center justify-center rounded">S</span>
                <span class="text-secondary-text text-sm">{{ position.sell_price > 0 ? position.sell_price : '--' }}</span>
              </div>
            </td>

            <!-- Qty -->
            <td class="py-2 text-secondary-text text-sm">{{ position.quantity }}</td>

            <!-- LTP -->
            <td class="py-2 text-secondary-text text-sm">{{ position.last_price }}</td>

            <!-- Status -->
            <td class="py-2">
              <span :class="statusClass(position.status)" class="text-white text-xs font-medium px-3 py-1 rounded">
                {{ position.status }}
              </span>
            </td>
            ...
            <!-- PNL -->
            <td class="py-2">
              <span :class="pnl(position) >= 0 ? 'text-primary-green' : 'text-primary-red'" class="text-sm font-semibold">
                {{ pnl(position) >= 0 ? '+' : '' }}{{ pnl(position).toFixed(2) }}
              </span>
            </td>

            <!-- Actions -->
            <td class="py-2 text-right">
              <button class="border border-primary-border text-secondary-text text-xs px-2 py-1.5 rounded-lg hover:bg-white/5 transition-colors">
                Sq.Off
              </button>
            </td>
          </tr>
          ...
          <tr v-if="!filteredPositions.length">
            <td colspan="8" class="py-10 text-center text-secondary-text text-sm">No positions found.</td>
          </tr>
        </template>
    ...
    <!-- Pagination -->
    <div class="flex items-center justify-between mt-5 pt-4 border-t border-primary-border">
      <p class="text-secondary-text text-xs">
        Showing {{ rangeStart }}–{{ rangeEnd }} of {{ store.pagination.total }} positions
      </p>
...
import { usePositionStore } from '../../stores/postion'

const store = usePositionStore()
const isStatusDropdownOpen = ref(false)
const selectedStatus = ref(null)
const statusOptions = ['closed', 'open', 'pending', 'partial']
...
const filteredPositions = computed(() => {
  if (!selectedStatus.value) return store.positions
  return store.positions.filter(position =>
    position.status.toLowerCase() === selectedStatus.value.toLowerCase()
  )
})

const changePage = (page) => {
  if (page < 1 || page > store.pagination.total_pages) return
  store.fetchPositions(page)
}

const pnl = (position) => {
  return (position.sell_price - position.buy_price) * position.quantity * position.multiplier
}

const statusClass = (status) => {
  const map = {
    CLOSED: 'bg-primary-red',
    OPEN: 'bg-primary-green',
